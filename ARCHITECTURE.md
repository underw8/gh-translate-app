...
完全にサーバーレスなGitHub Appで、プルリクエスト内のMarkdownファイルの変更を検出し、OpenAIで翻訳し、新しい翻訳PRを開きます。Webhookハンドラーとキューコンシューマーは、Cloudflare Workers上で完全に実行されます。
...
| サービス | 役割 | 理由 |
|---|---|---|
| **Workers** | Webhookハンドラー＋キューコンシューマー | サーバーレスHTTP＋イベント駆動アーキテクチャ |
| **Queues** | ハンドラーとコンシューマーを分離；長時間実行ジョブを処理 | GitHubの10秒webhookタイムアウトを回避；翻訳作業は非同期；最低1回配信保証 |
| **Workers Secrets** | APIキーとシークレットを保存 | 保存時に暗号化；実行時に`env`に挿入 |
| **Workers KV** *オプション* | べき等性ストア；翻訳済みPRを追跡 | 再試行されたキューメッセージから重複翻訳PRを防止 |

**なぜQueuesなのか、`ctx.waitUntil()`＋バックグラウンドフェッチではなく：** `waitUntil()`は再試行をサポートしていません。翻訳が途中で失敗すると、ジョブは失われます。Queuesは設定可能な再試行とデッドレターキュー（DLQ）を提供；コンシューマーは最大15分間で完了できます。
...
| 権限 | レベル | ユースケース |
|---|---|---|
| Pull requests | 読み取り＆書き込み | PRファイルをリスト；翻訳PRを開く |
| Contents | 読み取り＆書き込み | Markdownファイルを読み取り；翻訳コンテンツをコミット |
...
2. **JWTをインストールアクセストークンと交換**
   ...
   ```
   トークンは**1時間**で期限切れ；コンシューマー起動時に再生成します。
...
> **重要：** GitHub Appの秘密鍵は**PKCS#1形式**（`-----BEGIN RSA PRIVATE KEY-----`）でダウンロードされますが、`jose`は**PKCS#8**（`-----BEGIN PRIVATE KEY-----`）が必要です。保存前に変換してください：
> ```bash
...
### ステップ1：Webhookを受信 (Webhookハンドラーワーカー)

```
GitHub → POST /webhook
```

1. ボディをテキストとして読み取り（ストリームは1回だけ読み取り可能）
...
### ステップ2：キュージョブを処理 (キューコンシューマーワーカー)

1. **べき等性チェック**：KVで`{owner}/{repo}#${prNumber}`を検索。見つかった場合（翻訳済みPRが投稿済み）、`msg.ack()`して返す。
2. **認証**：App JWTを生成 – インストールアクセストークンと交換
3. **変更されたファイルをリスト**：`GET /repos/{owner}/{repo}/pulls/{prNumber}/files`
   - `.md`と`.mdx`ファイルで`status !== "removed"`をフィルタリング
...
- コンシューマーがクラッシュ – キューが自動的に再試行（`max_retries: 3`、`retry_delay: 30s`）
...
| 品質 | 技術文書に最適 | ニュアンスに対してより高い精度 |

推定コスト（ファイルあたり10Kトークン）：**~$0.007**（gpt-4o-mini）。
...
ほとんどのMarkdownファイルは~10Kトークン内に収まります – チャンキングは不要です。~110Kトークンを超えるファイルの場合：
1. 見出し境界で分割（`\n# `または`\n## `）
2. チャンクごとに最初の見出しのみを保持
3. 各チャンクを別々に翻訳
4. 順序どおりに連結
...
- ±25%のランダムジッターを追加；別のファイルリクエストを並列化
- ファイルあたり最大6回の再試行；超過した場合、ジョブを失敗（キューがメッセージ全体を再試行）

---

## 6. GitHub Git Database API – 翻訳をコミット

ブランチを作成してファイルをコミットするには、7つの順序付きGit APIコール（低レベルのGit操作）が必要です：
...
```jsonc
{
  ...
  "queues": {
    ...
    "consumers": [
      {
        "queue": "pr-translation-jobs",
        "max_batch_size": 1,          // 一度に1つのPRを処理（クォータ推定に予測可能）
...
| 決定 | 選択 | 代替案 | 根拠 |
|---|---|---|---|
| 非同期方法 | Cloudflare **Queues** | `ctx.waitUntil()` | Queuesは再試行＋DLQを提供；`waitUntil`はサイレント失敗 |
| 翻訳モデル | **gpt-4o-mini** | gpt-4o | 16倍安い；技術文書の品質は優秀 |
| JWTライブラリ | **`jose`** | ネイティブWeb Crypto | PKCS#8パース＋最小限のコードで標準準拠JWT |
| べき等性ストア | **Workers KV** | なし | Queuesは≥1配信を保証；KVは重複翻訳PRを防止 |
| ファイルコミット戦略 | **Git Database API**（7コール） | Octokit REST | Node.js依存なし；Workersランタイムで動作 |
| Webhookハンドラーレイテンシ | エンキュー＋`200`を即座に返す | インライン処理 | GitHubはwebhookを10sでタイムアウト；翻訳に分かかる |
| PRファイルメタデータ | コンシューマーでリスト化、webhookではない | キューメッセージで渡す | 初期メッセージサイズを削減；ファイルメタデータはPR内で安定 |
...
| 制約 | 値 | 影響 |
|---|---|---|
| Githubウェブフックタイムアウト | 10秒 | ハンドラーは翻訳開始前に`200`を返す必要 – Queuesが必須 |
| キューメッセージ最大サイズ | 128 KB | メタデータのみを保存；ファイルコンテンツはコンシューマーで取得 |
| Workers KV書き込み速度 | キーあたり最大1書き込み/秒 | べき等性書き込みはまれ – 問題なし |
| GitHub APIレート制限 | アプリトークンあたり5,000リクエスト/時間 | PRあたり~7APIコール – ~700PR/時間；通常は達しない |
| OpenAI TPM（Tier 1） | ~200Kトークン/分（gpt-4o-mini） | 最新バッチが役立つ；標準ストリーミング処理は一般的なドキュメント翻訳を実行 |
| GitHubコミットAPI速度 | 80リクエスト/分；500/時間 | PRあたり7コール – ~70PR/時間；ほぼすべてのリポジトリは制限内 |
| キュー再試行 | デフォルト3回、30秒遅延 | 一時的なOpenAI/GitHub エラーは自己修復；永続的エラー – DLQ |