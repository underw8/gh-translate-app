...
GitHub Appで着信なリクエストのたびのマークダウンファイルを自動翻訳し、翻訳済みファイルを含む新しいPRを開きます。OpenAIを使用したCloudflare Workersでテストされています。

## 動作方法

1. アプリがインストールされているリポジトリでPRを開くまたは更新する
...
npm run typecheck  # TypeScriptのチェック
npm test           # テスト実行
npm run lint       # リント
...
├── openai.ts         # OpenAI翻訳（gpt-4o-miniディスコンコンティニュアリーAPIロジック）
├── idempotency.ts    # Workers KVバックアップ（重複除去）
...
- [AGENTS.md](AGENTS.md) – 開発者ガイダンス、レイアウトコンセプト、レイアウターン、落とし穴とテスト概要
