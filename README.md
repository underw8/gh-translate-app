...
GitHub Appで、プルリクエスト内のマークダウン文書を自動翻訳し、翻訳されたファイルを含む新しいPRを開きます。OpenAIを使用し、Cloudflare Workersでホストされています。

## 動作方法

1. アプリがインストールされているリポジトリでPRが開かれるか更新される
2. アプリが変更された `.md` / `.mdx` ファイルを検出する
3. 各ファイルはOpenAI（`gpt-4o-mini`）を通じて翻訳される
4. 翻訳されたすべてのファイルを含む新しいPRが同じリポジトリで開かれる

## Tech Stack

- **Runtime**: [Cloudflare Workers](https://workers.cloudflare.com/)
- **非同期処理**: [Cloudflare Queues](https://developers.cloudflare.com/queues/)
...
**GitHub Settings → Developer settings → GitHub Apps → New GitHub App** に移動し、以下を設定します：
...
最後のコマンドで印刷されたKVネームスペースIDを `wrangler.jsonc` にコピーします。
...
[smee.io](https://smee.io/) や [ngrok](https://ngrok.com/) などのツールを使用して、GitHubウェブフックを `http://localhost:8787/webhook` に転送します。
...
├── queue.ts          # キューコンシューマー：翻訳フロー全体の調整
├── github.ts         # GitHub APIクライアント（認証、ファイルリスト、Git DB API、PR作成）
├── openai.ts         # OpenAI翻訳（gpt-4o-mini、チャンキング、リトライロジック）
├── idempotency.ts    # Workers KVヘルパー（重複排除）
...
- [ARCHITECTURE.md](ARCHITECTURE.md) – 完全な設計詳細、APIフロー、レート制限、トレードオフの決定
- [AGENTS.md](AGENTS.md) – 開発者およびエージェント向けガイド（パターン、落とし穴、テスト規約）
