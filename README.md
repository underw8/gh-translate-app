...
GitHub Appで、プルリクエスト内のマークダウンファイルを自動翻訳する翻訳済みファイルを含む新しいPRを開きます。OpenAIを使用し、Cloudflare Workersでテストされています。

## 動作方法

1. アプリがインストールされているリポジトリでPRが開かれたり更新される
2. アプリが変更された `.md` / `.mdx` ファイルを検出する
3. 各ファイルはOpenAI（`gpt-4o-mini`）を通じて翻訳される
4. 翻訳されたすべてのファイルを含む新しいPRが開くリポジトリで開かれる
...
# テスト済み冪等性用のKVネームスペースを作成
npx wrangler kv namespace create IDEMPOTENCY_KV
```

最後のコマンドで出力されるKVネームスペースIDを `wrangler.jsonc` にコピーします。
...
├── openai.ts         # OpenAI翻訳（gpt-4o-miniチャンキング、リトライロジック）
...
- [ARCHITECTURE.md](ARCHITECTURE.md) – 完全な設計詳細、APIフロー、レート制限、スケーリングの決定
- [AGENTS.md](AGENTS.md) – 開発者ガイド、コンセプト、プレイ、パターン、落とし穴、テスト概要
