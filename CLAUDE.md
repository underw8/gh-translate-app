...
GitHub App で、リクエストを受けて のワークダウンロード自動翻訳する翻訳済みファイルを含む新しい PR を開きます。OpenAI による駆動を Cloudflare Workers でテストしています。
...
- **Workers KV** — 短期ストア (キューメッセージ毎の重複翻訳 PR を防止)
...
// タイミングセーフ比較を使用してリクエスト値と hex(sig) を比較
```

### Webhook → キューのエンキュー
webhook ハンドラは GitHub の 10 秒タイムアウト前に `200 OK` を返す必要があります。すべての実際の処理はキューコンシューマで実行されます。
...
### GitHub App 認証
`jose` ライブラリ (PKCS#8 + RS256) を使用します。各キューコンシューマの起動時に新しく生成します (トークンの有効期限は 1 時間)。
...
> **PKCS#1 → PKCS#8 が必要:** GitHub キー PKCS#1 (`-----BEGIN RSA PRIVATE KEY-----`) でダウンロードされます。保存時に変換します:
> ```bash
...
### OpenAI 経由の翻訳
デフォルトモデル: **`gpt-4o-mini`** (128K コンテキスト、gpt-4o より約 16 倍価値)。`temperature: 0.1` を使用して翻訳の一貫性を保ちます。モデルに対してすべてのマークダウン構造 (frontmatter、コードブロック内容) を変更しないままで保存する明示的に指示します。429 エラーに対しては、エクスポーネンシャルバックオフ (6 回試行、1s→2s→4s→...60s キャップ ±25% ジッタ) で呼び出します。
...
6. `POST /git/refs` — ブランド作成 `translate/pr-{prNumber}`
7. `POST /pulls` — タイトル `[Translation] {LANG}: PR #{prNumber} — {title}` で PR を開く

### テスト差別化
作業を開始する前に、コンシューマは KV でキー `{owner}/{repo}#{prNumber}` をチェックします。見つかった場合、`msg.ack()` を実行して返します — ジョブ毎のキューの転送で既に完了しています。翻訳 PR が正常に開かれた後、PR URL を KV に書き込みます。
...
```
GITHUB_APP_ID=           # 数字の GitHub App ID
...
- テストで実際のネットワーク呼び出しを実行してはいけません。**いいえ**
...
- **シークレットキーの改行**: `\n` がデフォルトで保存されています (`cat | wrangler secret put` でファイルを使用)。実行時に復元: `.replace(/\\n/g, '\n')`。
...
- **テスト差別化**: キューは最大 1 回転送します。重複を避けるため、常に翻訳するブランチ/PR を作成する前に `IDEMPOTENCY_KV` をチェックしてください。
